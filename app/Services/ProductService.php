...
class ProductService implements ProductServiceInterface{
    ...
    public function productAdd($request){
        ...
        if(!empty($request['additional_imgs'])){
            ...
            return $product->additionalImages()->saveMany($additionalFiels);
        }
    ...
    /**
     * remove the product additional individual file from the storage
     * 
     * @return objects
     */
    public function removeIndividualImage($product, $imageId){
        $product = Product::findOrFail($product)->additionalImages()->where('id', $imageId)->first();
        $fileName = config('config.MAIN_UPATH').$product->image_name;
        if(!empty($fileName)){
            Storage::disk('local')->delete($fileName);
            $product->delete();
        }
